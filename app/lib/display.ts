...
export function toTaskDisplayMeta(task: {
  ...
  const statusLabel = statusMap[rawStatus] ?? (capitalize(rawStatus) || 'Unknown')

  return { durationLabel, focusLabel, focusVariant, scoreLabel, scoreVariant, effortLabel, statusLabel }
}
...
export function toProjectSummaryDisplay(project: {
  ...
  const statusMap: Record<string, { label: string; variant: ProjectSummaryDisplay['statusVariant'] }> = {
    'active':    { label: 'On track',  variant: 'success' },
    'blocked':   { label: 'Blocked',   variant: 'danger' },
    'at-risk':   { label: 'At risk',   variant: 'warning' },
    'completed': { label: 'Completed', variant: 'default' },
  }
  const mapped = statusMap[project.status ?? ''] ?? { label: 'Active', variant: 'success' as const }
...
function resolveSeverity(status: string | null | undefined, level: number | null | undefined): SeverityVariant {
...
export function toCodDisplayState(cod: {
  ...
  const constraintItems: CodDisplayState['constraintItems'] = Object.entries(cod.constraints ?? {}).map(
    ([k, v]) => ({ label: capitalize(k.replace(/_/g, ' ')), value: String(v ?? '') })
  )

  const signalItems: CodDisplayState['signalItems'] = Object.entries(cod.signals ?? {}).map(([k, v]) => {
...
function statusToVariant(status: string | null | undefined): NoteHeaderDisplay['statusVariant'] {
  switch (status) {
    case 'done':
...
function pathToBreadcrumbs(path: string | undefined): NoteHeaderDisplay['breadcrumbs'] {
  if (!path) return []
  ...
  const statusLabel = note.status ? (
    {
      ...
      'done': 'Done', 'backlog': 'Backlog', 'completed': 'Completed',
