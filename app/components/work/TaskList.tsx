...
// ---------------------------------------------------------------------------
// Memoized row — prevents re-render of all items when selection changes
// ---------------------------------------------------------------------------
...
const TaskItem = React.memo(function TaskItem({
  ...
  const handleClick = React.useCallback(() => {
    onSelect(isExpanded ? null : task);
  }, [isExpanded, onSelect, task]);

  return (
    <li className="rounded-2xl border border-border bg-[var(--surf-elevated)] shadow-sm">
      <button
        type="button"
        onClick={handleClick}
        aria-expanded={isExpanded}
        className={[
          'cursor-pointer flex w-full items-start justify-between rounded-2xl px-4 py-3 text-sm transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/30',
          isExpanded ? 'bg-surface3 text-text' : 'text-text2 hover:bg-surface',
        ].join(' ')}
      >
