...
export function HueyWorkspace({
  ...
  return (
    <SoftPanel className="h-full flex flex-col">
      {intentTemplate && (
        <div className="bg-primary/5 rounded-xl p-3 text-sm text-neutral-600 mb-4 shrink-0">
      ...
      <div ref={listRef} className="flex-1 min-h-0 overflow-y-auto space-y-4 mb-4">
        {messages.length === 0 && !loading && (
          <div className="flex flex-col items-center justify-center h-full gap-2 text-neutral-400 py-12">
        ...
        {messages.map((msg) => {
          if (msg.role === 'system') {
            return (
              <p key={msg.id} className="text-xs text-neutral-400 text-center py-2">
                {msg.content}
              </p>
            )
          }
          ...
          if (msg.role === 'user') {
            return (
              <div key={msg.id} className="flex justify-end">
                <div className="bg-neutral-100 rounded-2xl px-4 py-3 text-sm ml-auto max-w-[80%] text-right">
                  {msg.content}
                </div>
              </div>
            )
          ...
          return (
            <div key={msg.id} className="max-w-[85%]">
              <div
                className="bg-surface border border-neutral-200 rounded-2xl px-4 py-3 text-sm prose prose-sm max-w-none"
                // eslint-disable-next-line react/no-danger
                dangerouslySetInnerHTML={{ __html: renderMarkdown(msg.content) }}
              />
              {msg.meta && (
                <p className="text-xs text-neutral-400 mt-1 ml-2">{msg.meta}</p>
              )}
              <PostResponseActions />
            </div>
          )
        ...
        {loading && (
          <div className="max-w-[85%]">
            <div className="bg-surface border border-neutral-200 rounded-2xl px-4 py-3 text-sm text-neutral-400">
              Thinking…
            </div>
          </div>
        )}
      ...
      <div className="rounded-[28px] border border-neutral-200 bg-surface px-4 py-3 flex items-center gap-3 shrink-0">
        <textarea
