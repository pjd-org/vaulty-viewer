...
export interface CodDisplayState {
  ...
  severityVariant: 'clear' | 'warn' | 'rest' | 'stop'
  headline: string
  actionLabels: string[]
  constraintItems: { label: string; value: string }[]
  signalItems: { label: string; value: string; variant?: 'warn' | 'ok' | 'bad' }[]
  reasonText: string
}
...
export interface NoteHeaderDisplay {
  ...
  primaryActions: { label: string; variant: 'primary' | 'secondary'; action: string }[]
}
