...
const knowledgeSurfaceMock = vi.fn(
  ({
    ...
    <div data-testid="knowledge-surface">
      {noteId ?? 'no-note'}::{workspaceTo ?? 'no-route'}::{workspaceParams?.slug ?? 'no-slug'}
    </div>
  ),
)
...
describe('ProjectKnowledgeLaneShell', () => {
  it('renders the shared notes workspace inside the project lane and keeps route-aware links', () => {
    render(
    ...
    expect(screen.getByTestId('knowledge-surface').textContent).toContain(
      'notes/knowledge/human/engineering/typescript-guide.md::/project/$slug/knowledge::rent-stability-pantin',
    )
    ...
    expect(screen.getByRole('link', { name: 'Views' }).getAttribute('href')).toBe(
  ...
  it('renders placeholder lanes for views and memories', () => {
    render(<ProjectKnowledgeLaneShell slug="rent-stability-pantin" tab="views" />)
    expect(screen.getByText('Project Knowledge Views')).toBeTruthy()

    cleanup()

    render(<ProjectKnowledgeLaneShell slug="rent-stability-pantin" tab="memories" memoryTab="recent" />)
    expect(screen.getByText('Project Memories')).toBeTruthy()
    expect(screen.getByText('Agent and project memories will render here for recent.')).toBeTruthy()
  })
})
