...
describe('ProjectDetail (skeleton)', () => {
  ...
  it('renders the project board without loading skeleton when task cache is preloaded', async () => {
    const { ProjectDetailScene } = await import('../../app/components/projects/ProjectDetailScene');
    const qc = new QueryClient();
    ...
    qc.setQueryData(['viewer-adapter', 'project-surface', 'rent-stability-pantin'], {
      projectId: 'rent-stability-pantin',
      pressureBand: [],
      decisionQueue: [],
      immediateActions: [],
    ...
    expect(
      screen.getByText('Open the project board and execution queue.').closest('a')?.getAttribute('data-link-to'),
    ).toBe('/project/$slug/tasks');
    expect(
      screen.getByText('Jump to the project workspace and notes.').closest('a')?.getAttribute('data-link-to'),
    ).toBe('/project/$slug/knowledge');
    expect(
      screen.getByText('Inspect pipelines, runners, and schedules.').closest('a')?.getAttribute('data-link-to'),
      ...
      pressureBand: [
        {
          ...
          whySurfaced: 'This project is approaching a critical decision point.',
          allowedActions: [],
        },
      ],
      decisionQueue: [
      ...
      immediateActions: [
        {
          ...
          whyNow: 'The dossier can be assembled without waiting on other work.',
          expectedEffect: 'Moves the project toward a clean renewal decision.',
          confidence: 0.91,
          reversibility: 'high',
          mutationRef: {
            domain: 'work',
