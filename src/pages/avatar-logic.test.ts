...
describe("computeSnapshotStats", () => {
  it("safely derives vitals with defaults", () => {
    const result = computeSnapshotStats({
      vitals: { tasksCompletedToday: 3, sessionsCompletedThisWeek: 5, energy: 70 },
    });
    expect(result).toEqual({ tasksToday: 3, sessionsThisWeek: 5, energy: 70 });
  });

  it("defaults missing values to zero", () => {
    const result = computeSnapshotStats({});
    expect(result).toEqual({ tasksToday: 0, sessionsThisWeek: 0, energy: 0 });
  });
});
